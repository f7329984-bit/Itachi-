...
# =====================================================================
# 🌐 FLASK WEB SERVER FOR RENDER (CRITICAL)
# =====================================================================
app = Flask('')
...
# =====================================================================
# 🤖 BOT INIT WITH FLOOD WAIT HANDLING
# =====================================================================
bot = TelegramClient('itachi_bot', API_ID, API_HASH)
...
async def start_bot():
    ...
    try:
        ...
        print(f"⚠️ Flood wait required: {e.seconds} seconds")
        print(f"Waiting {e.seconds} seconds...")
        await asyncio.sleep(e.seconds)
        await bot.start(bot_token=BOT_TOKEN)
        print("✅ Bot started after flood wait!")
...
async def update_groups():
    ...
    try:
        ...
        print(f"✅ Updated groups: {len(GROUPS_CACHE)} groups")
    except Exception as e:
...
async def command_handler(event):
    ...
    if cmd in ["gc", "groupslist"]:
        ...
        for gid in GROUPS_CACHE[:20]:
            try:
                dialog = await bot.get_entity(gid)
                text += f"• `{gid}` - {getattr(dialog, 'title', 'Unknown')[:40]}\n"
            except:
                text += f"• `{gid}`\n"
        await event.reply(text)
        return
...
# =====================================================================
async def main():
    await start_bot()  # Start bot with flood wait handling
    await update_groups()
    me = await bot.get_me()
    print(f"""
╔══════════════════════════════╗
...
╚══════════════════════════════╝
  ...
  🔑 Sudo: {len(SUDO_USERS)}
  🌐 Web Server: Running on port {os.environ.get('PORT', 10000)}
  ✅ BOT IS ONLINE!
""")
